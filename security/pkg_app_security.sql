...
--   5. rec_dept_grant           — manual department overrides for users who need
--                                to see requisitions beyond their own as recruiter/HM.
--                                Queried directly by the VPD policy at runtime;
--                                no collection injection needed at login.
--
-- Function hierarchy:
...
--   REST userAccountRoles returns 25 roles for gcs_reports.
...
CREATE OR REPLACE PACKAGE BODY pkg_app_security AS
    ...
    BEGIN
        ...
        APEX_UTIL.SET_SESSION_STATE('G_DEPARTMENT_COUNT', l_loc_count);
    ...
    BEGIN
        ...
        APEX_UTIL.SET_SESSION_STATE('G_DEPARTMENT_COUNT', 0);
        ...
        APEX_UTIL.SET_SESSION_STATE('G_ASSIGNMENT_COUNT', l_asgt_count);
        -- G_DEPARTMENT_COUNT is set by an Application Computation (Before Header)
        -- on every page load so it reflects grant changes in real time.
    ...
    BEGIN
        ...
        -- =====================================================================
        -- STEP 2: Cache user assignments (position, location, department)
        -- =====================================================================
        cache_user_assignments(p_username);

        -- NOTE: Department grant overrides (rec_dept_grant) are no longer
        -- injected into the collection at login. The VPD policy (rec_rls_pkg)
        -- queries rec_dept_grant directly at query time, so no session state
        -- is required here.
