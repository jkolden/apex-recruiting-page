create or replace package body pkg_rest_recruiting as
    ...
    procedure load_candidates(p_full_refresh in boolean default false) is
        ...
        l_pass         number := 0;
    begin
        -- Incremental: start from max CandLastModifiedDate minus 4-hour overlap.
        -- CandLastModifiedDate is volatile (changes when applications, assessments,
        -- questionnaires etc. are updated), so a short overlap avoids re-processing
        -- most of the table on every run.  MERGE is idempotent so overlap is safe.
        -- NULL = full refresh (first run, empty table, or explicit full refresh).
        if not p_full_refresh then
            begin
                select to_char(
                           cast(max(candlastmodifieddate) as timestamp) - interval '4' hour,
                           'YYYY-MM-DD"T"HH24:MI:SS".000Z"'
                       )
                  into l_date_filter
        ...
        insert into bicc_load_log (
            ...
            case when l_date_filter is not null
                 then 'Incremental from ' || l_date_filter
                 else 'Full refresh'
            end
        );
        insert into bicc_load_log (
    ...
    -- LOAD_APPLICATIONS
    -- =========================================================================
    -- Consolidated loader: paginates recruitingJobApplications and MERGEs
    -- into JOB_APPLICANTS_R.  Replaces APEX declarative sync for this entity.
    -- Preserves APEX$ columns so RECRUITING_REPORT_V ROW_NUMBER dedup works.
    -- =========================================================================

    procedure load_applications(p_full_refresh in boolean default false) is
        l_url          varchar2(2000);
        l_body         clob;
        l_offset       number := 0;
        l_limit        number := 500;
        l_app_merged   number := 0;
        l_error_msg    varchar2(4000);
        l_date_filter  varchar2(200) := null;
    begin
        -- Incremental: filter by LastUpdateDate minus 4-hour overlap.
        -- MERGE is idempotent so short overlap is safe; avoids re-processing
        -- the bulk of the table when only a few records changed.
        if not p_full_refresh then
            begin
                select to_char(
                           cast(max(lastupdatedate) as timestamp) - interval '4' hour,
                           'YYYY-MM-DD"T"HH24:MI:SS".000Z"'
                       )
                  into l_date_filter
        ...
        loop
            ...
            -- Apply incremental date filter if set
            if l_date_filter is not null then
                l_url := l_url || '&q=LastUpdateDate+%3E+%27'
                    || utl_url.escape(
                           replace(l_date_filter, '+00:00', 'Z'),
                           true, 'AL32UTF8')
                    || '%27';
            ...
            ---------------------------------------------------------
            -- MERGE job applications into JOB_APPLICANTS_R
            ---------------------------------------------------------
            merge into job_applicants_r t
            using (
                select
                    jt.job_application_id,
                    jt.candidate_person_id,
                    jt.candidate_name,
                    jt.requisition_id,
                    jt.phase_id,
                    jt.phase_name,
                    jt.state_id,
                    jt.state_name,
                    jt.applied_to_other_jobs,
                    jt.confirmed_flag,
                    jt.disqualified_flag,
                    jt.internal_flag,
                    jt.language_code,
                    jt.profile_id,
                    jt.apply_flow_version_id,
                    jt.confirmed_by_person_id,
                    jt.esign_desc_version_id,
                    jt.legal_desc_version_id,
                    jt.process_id,
                    jt.public_state_id,
                    jt.public_state_name,
                    jt.site_number,
                    jt.hiring_manager_id,
                    jt.requisition_id_rest,
                    jt.requisition_usage_code,
                    jt.recruiter_id,
                    jt.requisition_number,
                    jt.created_by,
                    jt.last_updated_by,
                    jt.job_application_date,
                    jt.creation_date,
                    jt.last_update_date,
                    jt.last_modified_date,
                    jt.confirmed_date
                from json_table(l_body, '$.items[*]' columns (
                    job_application_id       number          path '$.JobApplicationId',
                    candidate_person_id      number          path '$.CandidatePersonId',
                    candidate_name           varchar2(240)   path '$.CandidateName',
                    requisition_id           number          path '$.RequisitionId',
                    phase_id                 number          path '$.PhaseId',
                    phase_name               varchar2(240)   path '$.PhaseName',
                    state_id                 number          path '$.StateId',
                    state_name               varchar2(240)   path '$.StateName',
                    applied_to_other_jobs    varchar2(255)   path '$.AppliedToOtherJobs',
                    confirmed_flag           varchar2(5)     path '$.ConfirmedFlag',
                    disqualified_flag        varchar2(5)     path '$.DisqualifiedFlag',
                    internal_flag            varchar2(5)     path '$.InternalFlag',
                    language_code            varchar2(4)     path '$.LanguageCode',
                    profile_id               number          path '$.ProfileId',
                    apply_flow_version_id    number          path '$.ApplyFlowVersionId',
                    confirmed_by_person_id   number          path '$.ConfirmedByPersonId',
                    esign_desc_version_id    number          path '$.EsignDescriptionVersionId',
                    legal_desc_version_id    number          path '$.LegalDescriptionVersionId',
                    process_id               number          path '$.ProcessId',
                    public_state_id          number          path '$.PublicStateId',
                    public_state_name        varchar2(240)   path '$.PublicStateName',
                    site_number              varchar2(240)   path '$.SiteNumber',
                    hiring_manager_id        number          path '$.HiringManagerId',
                    requisition_id_rest      number          path '$.RequisitionIdRest',
                    requisition_usage_code   varchar2(30)    path '$.RequisitionUsageCode',
                    recruiter_id             number          path '$.RecruiterId',
                    requisition_number       varchar2(240)   path '$.RequisitionNumber',
                    created_by               varchar2(64)    path '$.CreatedBy',
                    last_updated_by          varchar2(64)    path '$.LastUpdatedBy',
                    job_application_date     timestamp       path '$.JobApplicationDate',
                    creation_date            timestamp       path '$.CreationDate',
                    last_update_date         timestamp       path '$.LastUpdateDate',
                    last_modified_date       timestamp       path '$.LastModifiedDate',
                    confirmed_date           timestamp       path '$.ConfirmedDate'
                )) jt
                where jt.job_application_id is not null
            ) s on (t.jobapplicationid = s.job_application_id)
            when matched then update set
                t.candidatepersonid          = s.candidate_person_id,
                t.candidatename              = s.candidate_name,
                t.requisitionid              = s.requisition_id,
                t.phaseid                    = s.phase_id,
                t.phasename                  = s.phase_name,
                t.stateid                    = s.state_id,
                t.statename                  = s.state_name,
                t.appliedtootherjobs         = s.applied_to_other_jobs,
                t.confirmedflag              = s.confirmed_flag,
                t.disqualifiedflag           = s.disqualified_flag,
                t.internalflag               = s.internal_flag,
                t.languagecode               = s.language_code,
                t.profileid                  = s.profile_id,
                t.applyflowversionid         = s.apply_flow_version_id,
                t.confirmedbypersonid        = s.confirmed_by_person_id,
                t.esigndescriptionversionid  = s.esign_desc_version_id,
                t.legaldescriptionversionid  = s.legal_desc_version_id,
                t.processid                  = s.process_id,
                t.publicstateid              = s.public_state_id,
                t.publicstatename            = s.public_state_name,
                t.sitenumber                 = s.site_number,
                t.hiringmanagerid            = s.hiring_manager_id,
                t.requisitionidrest          = s.requisition_id_rest,
                t.requisitionusagecode       = s.requisition_usage_code,
                t.recruiterid                = s.recruiter_id,
                t.requisitionnumber          = s.requisition_number,
                t.createdby                  = s.created_by,
                t.lastupdatedby              = s.last_updated_by,
                t.jobapplicationdate         = s.job_application_date,
                t.creationdate               = s.creation_date,
                t.lastupdatedate             = s.last_update_date,
                t.lastmodifieddate           = s.last_modified_date,
                t.confirmeddate              = s.confirmed_date,
                t."APEX$ROW_SYNC_TIMESTAMP"  = systimestamp
            when not matched then insert (
                "APEX$RESOURCEKEY",
                jobapplicationid, candidatepersonid, candidatename,
                requisitionid, phaseid, phasename, stateid, statename,
                appliedtootherjobs, confirmedflag, disqualifiedflag, internalflag,
                languagecode, profileid, applyflowversionid,
                confirmedbypersonid, esigndescriptionversionid,
                legaldescriptionversionid, processid,
                publicstateid, publicstatename, sitenumber,
                hiringmanagerid, requisitionidrest, requisitionusagecode,
                recruiterid, requisitionnumber,
                createdby, lastupdatedby,
                jobapplicationdate, creationdate, lastupdatedate,
                lastmodifieddate, confirmeddate,
                "APEX$ROW_SYNC_TIMESTAMP"
            ) values (
                to_char(s.job_application_id),
                s.job_application_id, s.candidate_person_id, s.candidate_name,
                s.requisition_id, s.phase_id, s.phase_name, s.state_id, s.state_name,
                s.applied_to_other_jobs, s.confirmed_flag, s.disqualified_flag, s.internal_flag,
                s.language_code, s.profile_id, s.apply_flow_version_id,
                s.confirmed_by_person_id, s.esign_desc_version_id,
                s.legal_desc_version_id, s.process_id,
                s.public_state_id, s.public_state_name, s.site_number,
                s.hiring_manager_id, s.requisition_id_rest, s.requisition_usage_code,
                s.recruiter_id, s.requisition_number,
                s.created_by, s.last_updated_by,
                s.job_application_date, s.creation_date, s.last_update_date,
                s.last_modified_date, s.confirmed_date,
                systimestamp
            );

            l_app_merged := l_app_merged + sql%rowcount;
        ...
        insert into bicc_load_log (
            ...
            'REST_APPLICATIONS', 'REFRESH', l_app_merged, l_app_merged, 'SUCCESS',
            case when l_date_filter is not null
                 then 'Incremental from ' || l_date_filter
                 else 'Full refresh'
            end
    ...
    exception
        when others then
            ...
            insert into bicc_load_log (
                ...
                'REST_APPLICATIONS', 'REFRESH', 'ERROR', l_error_msg
            );
            commit;
            raise;
    end load_applications;
    ...
    procedure refresh_all(p_full_refresh in boolean default false) is
    begin
        load_requisitions(p_full_refresh => true);  -- always full (small table)
        load_candidates(p_full_refresh => p_full_refresh);
        load_applications(p_full_refresh => p_full_refresh);
    end refresh_all;
