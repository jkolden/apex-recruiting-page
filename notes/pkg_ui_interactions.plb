CREATE OR REPLACE PACKAGE BODY pkg_ui_interactions AS
            ...
            SELECT jt.*
              FROM json_table(l_body, '$.items[*]'
                  COLUMNS (
                      ...
                      added_by_person_id   NUMBER         PATH '$.AddedByPersonId'
                  )
              ) jt
             WHERE jt.context_type_code = 'ORA_SUBMISSION'
               AND jt.interaction_id IS NOT NULL
        ) LOOP
            -- Resolve AddedByPersonId → USER_NAME, fall back to DisplayName
            BEGIN
                SELECT e.user_name INTO l_created_by
                  FROM hcm_employee_bc e
                 WHERE e.person_id = r.added_by_person_id
                   AND ROWNUM = 1;
            EXCEPTION
                WHEN NO_DATA_FOUND THEN
                    l_created_by := r.display_name;
            END;
    ...
    -- Caller (scheduler job) must provide APEX session context.
    -- =========================================================================
    PROCEDURE sync_all
    IS
        l_count        NUMBER  := 0;
