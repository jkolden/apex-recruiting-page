-- =============================================================================
-- VIEW: RECRUITING_REPORT_V  (v2 -- dev2 migration)
-- =============================================================================
-- Changes from v1:
--   1. latest_person_data CTE -> ext_flex_recruiting_v (was ext_flex_person_data_v)
--      - rehire_eligible -> rehire_eligibility
--      - gcs_effective_date -> proposed_effective_date
--      - added: pay_grade, pay_step, processing_owner, contract_type,
--              teacher_years_of_experience, educator_id, cate_experience, fte
--   2. Removed latest_job_refs CTE + JOIN + 6 EFF_REFERENCE columns
--      (GCS Job Application References context no longer exists in dev2)
--   3. Retirement: added subject_to_earnings_limit, teri_begin_date, teri_end_date
--   4. teach_refs CTE -> ext_refs: question codes renamed in dev2
--      GCS_TEACH_REF_* -> GCS_REF_EXT_* / GCS_EXT_REF_*
--      Output aliases simplified: GCS_TEACH_REF_1_NAME -> REF_1_NAME, etc.
-- =============================================================================
...
latest_recruiting_data AS (
    SELECT rd.*,
           ROW_NUMBER() OVER (PARTITION BY rd.person_id
                              ORDER BY rd.effective_start_date DESC) AS rn
    FROM ext_flex_recruiting_v rd
),
...
ext_refs AS (
    SELECT
        SUBJECT_ID,
        MAX(CASE WHEN QUESTION_CODE = 'GCS_REF_EXT_NAME1'   THEN ANSWER_TEXT END) AS REF_1_NAME,
        MAX(CASE WHEN QUESTION_CODE = 'GCS_EXT_REF_PHONE1'  THEN ANSWER_TEXT END) AS REF_1_PHONE,
        MAX(CASE WHEN QUESTION_CODE = 'GCS_REF_EXT_EMAIL1'  THEN ANSWER_TEXT END) AS REF_1_EMAIL,
        MAX(CASE WHEN QUESTION_CODE = 'GCS_REF_EXT_REL1'    THEN ANSWER_TEXT END) AS REF_1_REL,
        MAX(CASE WHEN QUESTION_CODE = 'GCS_EXT_REF_NAME2'   THEN ANSWER_TEXT END) AS REF_2_NAME,
        MAX(CASE WHEN QUESTION_CODE = 'GCS_EXT_REF_PHONE2'  THEN ANSWER_TEXT END) AS REF_2_PHONE,
        MAX(CASE WHEN QUESTION_CODE = 'GCS_EXT_REF_EMAIL2'  THEN ANSWER_TEXT END) AS REF_2_EMAIL,
        MAX(CASE WHEN QUESTION_CODE = 'GCS_EXT_REF_REL2'    THEN ANSWER_TEXT END) AS REF_2_REL,
        MAX(CASE WHEN QUESTION_CODE = 'GCS_EXT_REF_NAME3'   THEN ANSWER_TEXT END) AS REF_3_NAME,
        MAX(CASE WHEN QUESTION_CODE = 'GCS_EXT_REF_PHONE3'  THEN ANSWER_TEXT END) AS REF_3_PHONE,
        MAX(CASE WHEN QUESTION_CODE = 'GCS_EXT_REF_EMAIL3'  THEN ANSWER_TEXT END) AS REF_3_EMAIL,
        MAX(CASE WHEN QUESTION_CODE = 'GCS_EXT_REF_REL3'    THEN ANSWER_TEXT END) AS REF_3_REL
    FROM FBX_QSTNR_V
    WHERE QUESTION_CODE IN (
        'GCS_REF_EXT_NAME1','GCS_EXT_REF_PHONE1','GCS_REF_EXT_EMAIL1','GCS_REF_EXT_REL1',
        'GCS_EXT_REF_NAME2','GCS_EXT_REF_PHONE2','GCS_EXT_REF_EMAIL2','GCS_EXT_REF_REL2',
        'GCS_EXT_REF_NAME3','GCS_EXT_REF_PHONE3','GCS_EXT_REF_EMAIL3','GCS_EXT_REF_REL3'
    )
    GROUP BY SUBJECT_ID
),
survey_status AS (
...
SELECT
    ...
    /* -- EFF: GCS Recruiting Details -- */
    rd.rehire_eligibility               AS REHIRE_ELIGIBLE,
    rd.reference_check,
    rd.sled                             AS BACKGROUND_CHECK,
    rd.certification,
    rd.work_keys,
    rd.teacher_assessment_score         AS ASSESSMENT_SCORE,
    rd.interview_notes,
    rd.teacher_subject_area,
    rd.proposed_effective_date          AS EFF_EFFECTIVE_DATE,
    rd.pay_grade,
    rd.pay_step,
    rd.processing_owner,
    rd.contract_type,
    rd.teacher_years_of_experience,
    rd.educator_id,
    rd.cate_experience,
    rd.fte,
    ...
    /* -- Questionnaire: External References (COALESCE correction over original) -- */
    COALESCE(rc.REF_1_NAME,  er.REF_1_NAME)                       AS GCS_TEACH_REF_1_NAME,
    fmt_phone(COALESCE(rc.REF_1_PHONE, er.REF_1_PHONE))           AS GCS_TEACH_REF_1_PHONE,
    LOWER(TRIM(COALESCE(rc.REF_1_EMAIL, er.REF_1_EMAIL)))         AS GCS_TEACH_REF_1_EMAIL,
    er.REF_1_REL                                                   AS GCS_TEACH_REF_1_REL,
    COALESCE(rc.REF_2_NAME,  er.REF_2_NAME)                       AS GCS_TEACH_REF_2_NAME,
    fmt_phone(COALESCE(rc.REF_2_PHONE, er.REF_2_PHONE))           AS GCS_TEACH_REF_2_PHONE,
    LOWER(TRIM(COALESCE(rc.REF_2_EMAIL, er.REF_2_EMAIL)))         AS GCS_TEACH_REF_2_EMAIL,
    er.REF_2_REL                                                   AS GCS_TEACH_REF_2_REL,
    COALESCE(rc.REF_3_NAME,  er.REF_3_NAME)                       AS GCS_TEACH_REF_3_NAME,
    fmt_phone(COALESCE(rc.REF_3_PHONE, er.REF_3_PHONE))           AS GCS_TEACH_REF_3_PHONE,
    LOWER(TRIM(COALESCE(rc.REF_3_EMAIL, er.REF_3_EMAIL)))         AS GCS_TEACH_REF_3_EMAIL,
    er.REF_3_REL                                                   AS GCS_TEACH_REF_3_REL,
    ...
    CASE WHEN INSTR(',' || ss.SURVEY_EMAILS || ',',
        ',' || LOWER(TRIM(COALESCE(rc.REF_1_EMAIL, er.REF_1_EMAIL))) || ',') > 0
        THEN 'Y' ELSE 'N' END                               AS REF_1_SURVEY_YN,
    CASE WHEN INSTR(',' || ss.SURVEY_EMAILS || ',',
        ',' || LOWER(TRIM(COALESCE(rc.REF_2_EMAIL, er.REF_2_EMAIL))) || ',') > 0
        THEN 'Y' ELSE 'N' END                               AS REF_2_SURVEY_YN,
    CASE WHEN INSTR(',' || ss.SURVEY_EMAILS || ',',
        ',' || LOWER(TRIM(COALESCE(rc.REF_3_EMAIL, er.REF_3_EMAIL))) || ',') > 0
...
LEFT JOIN latest_recruiting_data rd
    ON rd.person_id = app.CANDIDATEPERSONID AND rd.rn = 1
...
LEFT JOIN ext_refs er
    ON er.SUBJECT_ID = app.JOBAPPLICATIONID
